...
contract AlphaSwapPair is AlphaSwapERC20, ReentrancyGuard {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Only AlphaSwapFactory can call this function");
        token0 = _token0;
        token1 = _token1;
    ...
    function addLiquidity(
        ...
        if (balance0 == 0 && balance1 == 0) {
            ...
            require(
                IERC20(_token0).transferFrom(_provider, address(this), _amount0),
                "Payment of token0 didn't go through"
                );
            require(
                IERC20(_token1).transferFrom(_provider, address(this), _amount1), 
    ...
    function _mintForProvider(address _provider, uint _amount0, uint _amount1) private {
        uint totalSup = totalSupply();
        uint liq = SMath.min(_amount0 * totalSup / balance0, _amount1 * totalSup / balance1);
        _mint(_provider, liq);
        emit Mint(_provider, liq);
    }
    ...
    function swap(address _tokenToSwap, uint _amountToSwap, address _sender) external nonReentrant {
        ...
        // Send funds
        require(
            IERC20(_tokenToSwap).transferFrom(_sender, address(this), _amountToSwap),
    ...
    function getBalance() public view returns (uint, uint, address, address) {
