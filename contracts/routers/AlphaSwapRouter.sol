...
contract AlphaSwapRouter {
    ...
    function getAddress(address _token0, address _token1) public view returns (bool, address) {
        (bool exists, address pairAddress) = factory.checkIfPairExists(_token0, _token1);
        return (exists, pairAddress);
    }
    
    
    
    function swap(address _tokenToSwap, address _tokenToReceive,  uint _amountToSwap) public {
        (bool exists, address pairAddress) = getAddress(_tokenToSwap, _tokenToReceive);
        require(exists, "This pair doesn't exist");
        emit Swap(msg.sender, pairAddress);
        AlphaSwapPair(pairAddress).swap(_tokenToSwap, _amountToSwap, msg.sender);
    }


    function provideLiquidity(address _token0, address _token1, uint _amount0, uint _amount1) public {
        ...
        AlphaSwapPair(pairAddress).addLiquidity(msg.sender, _token1, _token0, _amount1, _amount0);
    ...
    function createPair(address _token0, address _token1) public returns(address) {
        require(_token0 != address(0) && _token1 != address(0), "token can't be a zero address");
