...
contract AlphaSwapFactory {
    ...
    function checkIfPairExists(address _token0, address _token1) public view returns (bool, address){
        if (pairAddress[_token0][_token1] == address(0) && pairAddress[_token1][_token0] == address(0)) {
    ...
    function createPair(address _token0, address _token1) external returns(address) {
        require(_token0 != address(0) && _token1 != address(0), "toknes can't equal to the zero address");
